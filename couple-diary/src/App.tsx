...
const Card = ({
  ...
  <div
    style={{
      ...
      borderRadius: 22,
      padding: "20px",
      boxShadow: "0 2px 16px rgba(61,34,24,.07)",
      marginBottom: 16,
      width: "100%",
      maxWidth: "100%",
      overflow: "hidden",
...
const EmptyState = ({ emoji, text }: { emoji: string; text: string }) => (
  <div style={{ textAlign: "center", padding: "48px 18px", color: COLORS.muted }}>
...
const ActionButtons = ({
  ...
  <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end" }}>
    <Btn small outline color={COLORS.blue} onClick={onEdit}>
      编辑
    </Btn>
    <Btn small outline color={COLORS.danger} onClick={onDelete}>
      删除
    </Btn>
  </div>
);
...
const FormActions = ({
  ...
  <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
    <Btn onClick={onSave} color={color}>
      {saveText}
    </Btn>
    <Btn onClick={onCancel} outline color={COLORS.muted}>
      取消
    </Btn>
  </div>
);
    ...
    <div>
      ...
      <Card>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <button onClick={() => setViewDate(new Date(year, month - 1))} style={{ background: "none", border: "none", fontSize: 24, cursor: "pointer" }}>‹</button>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  borderRadius: 11,
                  cursor: "pointer",
                  background: isSelected ? COLORS.primary : isToday ? COLORS.soft : "transparent",
      ...
      <div style={{ display: "flex", gap: 7, marginBottom: 12, flexWrap: "wrap" }}>
        {EMOJIS.map((e) => (
          <span
            ...
            style={{ fontSize: 25, cursor: "pointer", opacity: value.emoji === e ? 1 : 0.35 }}
          >
            {e}
          </span>
        ))}
  ...
  const Fields = ({
    ...
    <>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center", flexWrap: "wrap" }}>
        ...
        {PRIORITY.map((p) => (
          <span key={p} onClick={() => setValue((old) => ({ ...old, priority: p }))} style={{ fontSize: 24, cursor: "pointer", opacity: value.priority === p ? 1 : 0.35 }}>
            {p}
          </span>
        ))}
      </div>
    ...
    setData((prev) =>
      prev.map((i) => {
        ...
        if (!movedToAnotherDay) {
          return { ...i, ...editForm, updatedAt: now() };
        }
    ...
    <>
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
        {DAYS_ZH.map((d) => (
          <span
            key={d}
            onClick={() => setValue((p) => ({ ...p, day: d }))}
            style={{
              padding: "6px 12px",
              borderRadius: 999,
              background: value.day === d ? COLORS.blue : COLORS.light,
              color: value.day === d ? "#fff" : COLORS.muted,
              cursor: "pointer",
              fontSize: 14,
              fontWeight: 800,
            }}
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
        {TIME_BLOCKS.map((block) => (
          <span
            key={block}
            onClick={() => setValue((p) => ({ ...p, period: block }))}
            style={{
              padding: "6px 12px",
              borderRadius: 999,
              background: value.period === block ? COLORS.primary : COLORS.light,
              color: value.period === block ? "#fff" : COLORS.muted,
              cursor: "pointer",
              fontSize: 14,
              fontWeight: 800,
            }}
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {WHO.map((w) => (
            <span
              key={w}
              onClick={() => setValue((p) => ({ ...p, who: w }))}
              style={{
                padding: "6px 12px",
                borderRadius: 999,
                background: value.who === w ? COLORS.primary : COLORS.light,
                color: value.who === w ? "#fff" : COLORS.muted,
                cursor: "pointer",
                fontSize: 14,
                fontWeight: 800,
              }}
                  ...
                  {editId === item.id ? (
                    ...
                    <>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                        {inferSchedulePeriod(item) && <span style={{ fontSize: 14, color: COLORS.muted }}>{inferSchedulePeriod(item)}</span>}
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                          <button
                            type="button"
                            aria-label="上移"
                            onClick={index === 0 ? undefined : () => moveScheduleItem(day, item.id, "up")}
                            style={{
                              width: 28,
                              height: 28,
                              borderRadius: "50%",
                          ...
                          <button
                            type="button"
                            aria-label="下移"
                            onClick={index === grouped[day].length - 1 ? undefined : () => moveScheduleItem(day, item.id, "down")}
                            style={{
                              width: 28,
                              height: 28,
                              borderRadius: "50%",
                              ...
                              background: "transparent",
                              color: COLORS.muted,
                              cursor: index === grouped[day].length - 1 ? "default" : "pointer",
                              opacity: index === grouped[day].length - 1 ? 0.28 : 1,
                              fontSize: 15,
                              fontWeight: 900,
                              lineHeight: 1,
                              padding: 0,
                        ...
                        <ActionButtons
                          onEdit={() => {
                            ...
                            setEditForm({
                              day: item.day,
                              period: inferSchedulePeriod(item) || "上午",
                              time: item.time,
                              event: item.event,
                              who: item.who,
                            });
    ...
    <>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 12, flexWrap: "wrap" }}>
        {URGENCY.map((u) => (
          <span
            key={u}
            onClick={() => setValue((p) => ({ ...p, urgency: u }))}
            style={{
              padding: "6px 14px",
              borderRadius: 999,
              background: value.urgency === u ? COLORS.primary : COLORS.light,
              color: value.urgency === u ? "#fff" : COLORS.muted,
              cursor: "pointer",
              fontSize: 14,
              fontWeight: 800,
            }}
        ...
        return (
          <Card key={item.id} style={{ borderLeft: `4px solid ${COLORS.green}` }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 10, marginBottom: 14 }}>
              {current === 0 && (
                <div style={{ background: COLORS.light, borderRadius: 16, padding: 12 }}>
                  <div style={{ color: COLORS.muted, fontSize: 13, fontWeight: 800 }}>离上次打卡</div>
                  <div style={{ color: COLORS.text, fontSize: 22, fontWeight: 900 }}>{lastGap === null ? "—" : `${lastGap} 天`}</div>
                </div>
              )}
              <div style={{ background: COLORS.light, borderRadius: 16, padding: 12 }}>
                <div style={{ color: COLORS.muted, fontSize: 13, fontWeight: 800 }}>已坚持</div>
                <div style={{ color: COLORS.text, fontSize: 22, fontWeight: 900 }}>{current} 天</div>
              </div>
              <div style={{ background: COLORS.light, borderRadius: 16, padding: 12 }}>
                <div style={{ color: COLORS.muted, fontSize: 13, fontWeight: 800 }}>最长坚持</div>
                <div style={{ color: COLORS.text, fontSize: 22, fontWeight: 900 }}>{item.longestStreak || 0} 天</div>
              </div>
              <div style={{ background: COLORS.light, borderRadius: 16, padding: 12 }}>
                <div style={{ color: COLORS.muted, fontSize: 13, fontWeight: 800 }}>累计打卡</div>
                <div style={{ color: COLORS.text, fontSize: 22, fontWeight: 900 }}>{item.totalCheckins || 0} 次</div>
              </div>
            </div>

            <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
              <Btn onClick={() => checkIn(item.id)} color={checkedToday ? COLORS.muted : COLORS.green} small>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {CATEGORIES.map((c) => (
            <span
              key={c}
              onClick={() => setValue((p) => ({ ...p, category: c }))}
              style={{
                padding: "6px 12px",
                borderRadius: 999,
                background: value.category === c ? COLORS.green : COLORS.light,
                color: value.category === c ? "#fff" : COLORS.muted,
                cursor: "pointer",
                fontSize: 14,
                fontWeight: 800,
              }}
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center", marginBottom: 18 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end" }}>
          {bought.length > 0 && (
            <Btn onClick={clearBought} small outline color={COLORS.danger}>
              清空已买
            </Btn>
          )}
          <Btn onClick={() => setAdding(!adding)} small color={COLORS.green}>
      ...
      {todo.length > 0 && (
        <div>
          <div style={{ fontWeight: 900, color: COLORS.green, fontSize: 16, margin: "0 0 8px" }}>待买</div>
      ...
      {bought.length > 0 && (
        <div>
          <div style={{ fontWeight: 900, color: COLORS.muted, fontSize: 16, margin: "16px 0 8px" }}>已买</div>
...
export default function CoupleDiary() {
  ...
  const renderTab = () => {
    switch (activeTab) {
      case "diary":
        return <DiaryTab data={diary} setData={setDiary} />;
      case "checkin":
        return <CheckinTab data={checkins} setData={setCheckins} />;
      case "schedule":
        return <ScheduleTab data={schedule} setData={setSchedule} />;
      case "shopping":
        return <ShoppingTab data={shopping} setData={setShopping} />;
      case "reminder":
        return <ReminderTab data={reminders} setData={setReminders} />;
      case "whisper":
        return <WhisperTab data={whispers} setData={setWhispers} />;
      case "jokes":
        return <JokesTab data={jokes} setData={setJokes} />;
      case "calendar":
        return <CalendarTab data={calendarData} setData={setCalendarData} />;
      case "wishes":
        return <WishesTab data={wishes} setData={setWishes} />;
      default:
        return null;
    }
  };
  ...
  return (
    <div
      style={{
        ...
        overflowX: "hidden",
        background: COLORS.bg,
        fontFamily: "'Georgia', 'Noto Serif SC', 'PingFang SC', 'Microsoft YaHei', serif",
        fontSize: 17,
        color: COLORS.text,
      }}
    >
      ...
      >
        <div style={{ fontSize: 13, color: "rgba(255,255,255,.75)", letterSpacing: 3, marginBottom: 4 }}>
          OUR LITTLE WORLD
        </div>
        <h1 style={{ margin: 0, color: "#fff", fontSize: 28, fontWeight: 900, letterSpacing: 1 }}>
          💑 我们的日记本
        </h1>
      </div>
      ...
      <div
        ...
        style={{
          ...
          gap: 0,
          background: COLORS.card,
          borderBottom: `2px solid ${COLORS.soft}`,
          position: "sticky",
          top: 91,
          zIndex: 99,
      ...
      >
        {TABS.map((tab) => (
          ...
          >
            ...
            <span style={{ fontSize: 12, fontWeight: 900, marginTop: 3, whiteSpace: "nowrap" }}>{tab.label}</span>
          </button>
        ))}
      </div>
